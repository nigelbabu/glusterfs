...
static inline uint32_t gf_flags_from_flags(uint32_t flags) {
  uint32_t gf_flags = 0;
...
static inline uint32_t gf_flags_to_flags(uint32_t gf_flags) {
  uint32_t flags = 0;
...
static inline void gf_statfs_to_statfs(struct gf_statfs *gf_stat,
                                       struct statvfs *stat) {
  if (!stat || !gf_stat)
    return;
...
static inline void gf_statfs_from_statfs(struct gf_statfs *gf_stat,
                                         struct statvfs *stat) {
  if (!stat || !gf_stat)
    return;
...
gf_proto_recall_lease_to_upcall(struct gfs3_recall_lease_req *recall_lease,
  ...
  GF_PROTOCOL_DICT_UNSERIALIZE(THIS, tmp->dict, (recall_lease->xdata).xdata_val,
                               (recall_lease->xdata).xdata_len, ret, errno,
                               out);
out:
  return ret;
}
...
gf_proto_recall_lease_from_upcall(xlator_t *this,
  ...
  GF_PROTOCOL_DICT_SERIALIZE(this, tmp->dict, &(recall_lease->xdata).xdata_val,
...
gf_proto_flock_to_flock(struct gf_proto_flock *gf_proto_flock,
  ...
  if (gf_flock->l_owner.len && (gf_flock->l_owner.len < GF_MAX_LOCK_OWNER_LEN))
...
static inline void gf_stat_to_iatt(struct gf_iatt *gf_stat, struct iatt *iatt) {
  if (!iatt || !gf_stat)
    return;
...
static inline void gf_stat_from_iatt(struct gf_iatt *gf_stat,
                                     struct iatt *iatt) {
  if (!iatt || !gf_stat)
    return;
...
static inline int gf_proto_cache_invalidation_from_upcall(
  ...
  is_cache_inval =
      ((gf_up_data->event_type == GF_UPCALL_CACHE_INVALIDATION) ? 1 : 0);
...
static inline void gfx_stat_to_iattx(struct gfx_iattx *gf_stat,
                                     struct iatt *iatt) {
  if (!iatt || !gf_stat)
    return;
...
static inline void gfx_stat_from_iattx(struct gfx_iattx *gf_stat,
                                       struct iatt *iatt) {
  if (!iatt || !gf_stat)
    return;
...
/* dict_to_xdr () */
static inline int dict_to_xdr(dict_t *this, gfx_dict *dict) {
  int ret = -1;
  int i = 0;
  int index = 0;
  ...
  dict->pairs.pairs_val =
      GF_CALLOC(1, (this->count * sizeof(gfx_dict_pair)), gf_common_mt_char);
  ...
  for (i = 0; i < this->count; i++) {
    ...
    case GF_DATA_TYPE_INT:
      index++;
      xpair->value.gfx_value_u.value_int = strtoll(dpair->value->data, NULL, 0);
      break;
    ...
    case GF_DATA_TYPE_DOUBLE:
      index++;
      xpair->value.gfx_value_u.value_dbl = strtod(dpair->value->data, NULL);
      break;
    ...
    case GF_DATA_TYPE_PTR:
      ...
      gf_msg("dict", GF_LOG_INFO, EINVAL, LG_MSG_DICT_SERIAL_FAILED,
             "key '%s' would not be sent on wire in the future", dpair->key);
      break;
    default:
...
static inline int xdr_to_dict(gfx_dict *dict, dict_t **to) {
  int ret = -1;
  int index = 0;
  char *key = NULL;
  char *value = NULL;
  ...
  for (index = 0; index < dict->pairs.pairs_len; index++) {
    ...
    switch (xpair->value.type) {
      /* Add more type here */
    case GF_DATA_TYPE_INT:
      ret = dict_set_int64(this, key, xpair->value.gfx_value_u.value_int);
      break;
    case GF_DATA_TYPE_UINT:
      ret = dict_set_uint64(this, key, xpair->value.gfx_value_u.value_uint);
      break;
    case GF_DATA_TYPE_DOUBLE:
      ret = dict_set_double(this, key, xpair->value.gfx_value_u.value_dbl);
      break;
    case GF_DATA_TYPE_STR:
    ...
    if (ret) {
      gf_msg_debug(THIS->name, ENOMEM, "failed to set the key (%s) into dict",
                   key);
    }
