...
int rpcsvc_need_drc(rpcsvc_request_t *req);

drc_cached_op_t *rpcsvc_drc_lookup(rpcsvc_request_t *req);

int rpcsvc_send_cached_reply(rpcsvc_request_t *req, drc_cached_op_t *reply);

int rpcsvc_cache_reply(rpcsvc_request_t *req, struct iobref *iobref,
                       struct iovec *rpchdr, int rpchdrcount,
                       struct iovec *proghdr, int proghdrcount,
                       struct iovec *payload, int payloadcount);

int rpcsvc_cache_request(rpcsvc_request_t *req);

int32_t rpcsvc_drc_priv(rpcsvc_drc_globals_t *drc);

int rpcsvc_drc_init(rpcsvc_t *svc, dict_t *options);

int rpcsvc_drc_deinit(rpcsvc_t *svc);
