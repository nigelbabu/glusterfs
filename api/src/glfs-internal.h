...
static inline void *__glfs_calloc(size_t nmemb, size_t size,
                                  glfs_mem_release_t release, uint32_t type,
                                  const char *typestr) {
...
static inline void *__glfs_malloc(size_t size, glfs_mem_release_t release,
...
static inline void *__glfs_realloc(void *ptr, size_t size) {
...
static inline void __glfs_free(void *free_ptr) {
  struct glfs_mem_header *header = NULL;
...
fd_t *__glfs_migrate_fd(struct glfs *fs, xlator_t *subvol,
                        struct glfs_fd *glfd);
...
#define __GLFS_ENTRY_VALIDATE_FD(glfd, label)                                  \
  do {                                                                         \
    if (!glfd || !glfd->fd || !glfd->fd->inode || glfd->state != GLFD_OPEN) {  \
      errno = EBADF;                                                           \
      goto label;                                                              \
    }                                                                          \
    old_THIS = THIS;                                                           \
...
/*
  ...
  rpc replies.
*/
static inline int glfs_lock(struct glfs *fs, gf_boolean_t wait_for_migration) {
...
static inline void glfs_unlock(struct glfs *fs) {
...
int glfs_loc_touchup(loc_t *loc) GFAPI_PRIVATE(glfs_loc_touchup, 3.4.0);
...
int glfs_recall_lease_fd(struct glfs *fs, struct gf_upcall *up_data);

int glfs_recall_lease_upcall(struct glfs *fs, struct glfs_upcall *up_arg,
                             struct gf_upcall *up_data);

int glfs_get_upcall_cache_invalidation(struct gf_upcall *to_up_data,
                                       struct gf_upcall *from_up_data);
int glfs_h_poll_cache_invalidation(struct glfs *fs, struct glfs_upcall *up_arg,
                                   struct gf_upcall *upcall_data);

ssize_t glfs_anonymous_preadv(struct glfs *fs, struct glfs_object *object,
                              const struct iovec *iovec, int iovcnt,
                              off_t offset, int flags);
ssize_t glfs_anonymous_pwritev(struct glfs *fs, struct glfs_object *object,
                               const struct iovec *iovec, int iovcnt,
                               off_t offset, int flags);

struct glfs_object *glfs_h_resolve_symlink(struct glfs *fs,
                                           struct glfs_object *object);
...
struct dirent *glfs_readdirbuf_get(struct glfs_fd *glfd);

gf_dirent_t *glfd_entry_next(struct glfs_fd *glfd, int plus);

void gf_dirent_to_dirent(gf_dirent_t *gf_dirent, struct dirent *dirent);

void gf_lease_to_glfs_lease(struct gf_lease *gf_lease,
                            struct glfs_lease *lease);

void glfs_lease_to_gf_lease(struct glfs_lease *lease,
                            struct gf_lease *gf_lease);

void glfs_release_upcall(void *ptr);

int get_fop_attr_glfd(dict_t **fop_attr, struct glfs_fd *glfd);

int set_fop_attr_glfd(struct glfs_fd *glfd);

int get_fop_attr_thrd_key(dict_t **fop_attr);

void unset_fop_attr(dict_t **fop_attr);
