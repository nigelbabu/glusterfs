...
int mnt3_auth_set_netgroups_auth(struct mnt3_auth_params *aps,
                                 const char *filename);

/* Set the exports file to use in the auth */
int mnt3_auth_set_exports_auth(struct mnt3_auth_params *aps,
                               const char *filename);

/* Check if a host is authorized to perform a mount / nfs-fop */
int mnt3_auth_host(const struct mnt3_auth_params *aps, const char *host,
                   struct nfs3_fh *fh, const char *dir,
                   gf_boolean_t is_write_op, struct export_item **save_item);

/* Free resources used by the auth params struct */
void mnt3_auth_params_deinit(struct mnt3_auth_params *aps);

int mnt3_auth_fop_options_verify(const struct mnt3_auth_params *auth_params,
