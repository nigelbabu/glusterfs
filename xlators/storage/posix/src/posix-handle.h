...
#define HANDLE_ABSPATH_LEN(this)                                               \
  (POSIX_BASE_PATH_LEN(this) + SLEN("/" GF_HIDDEN_PATH "/00/00/" UUID0_STR) + 1)
...
#define MAKE_ENTRY_HANDLE(entp, parp, this, loc, ent_p)                        \
  do {                                                                         \
    ...
    if (LOC_HAS_ABSPATH(loc)) {                                                \
      ...
      __parp = strdupa(entp);                                                  \
      parp = dirname(__parp);                                                  \
      op_ret = posix_pstat(this, loc->inode, NULL, entp, ent_p, _gf_false);    \
      break;                                                                   \
    }                                                                          \
    errno = 0;                                                                 \
    ...
    if (errno != ELOOP) {                                                      \
      ...
      if (!parp || !entp) {                                                    \
        gf_msg(this->name, GF_LOG_ERROR, errno, P_MSG_ENTRY_HANDLE_CREATE,     \
...
int posix_handle_gfid_path(xlator_t *this, uuid_t gfid, const char *basename,
                           char *buf, size_t len);

int posix_handle_hard(xlator_t *this, const char *path, uuid_t gfid,
                      struct stat *buf);

int posix_handle_soft(xlator_t *this, const char *real_path, loc_t *loc,
                      uuid_t gfid, struct stat *buf);

int posix_handle_unset(xlator_t *this, uuid_t gfid, const char *basename);

int posix_create_link_if_gfid_exists(xlator_t *this, uuid_t gfid,
                                     char *real_path, inode_table_t *itable);

int posix_check_internal_writes(xlator_t *this, fd_t *fd, int sysfd,
                                dict_t *xdata);
