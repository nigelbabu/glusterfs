...
#define MAKE_INODE_HANDLE(rpath, this, loc, iatt_p)                            \
  do {                                                                         \
    if (!this->private) {                                                      \
      op_ret = -1;                                                             \
      gf_msg("make_inode_handle", GF_LOG_ERROR, 0, P_MSG_INODE_HANDLE_CREATE,  \
    ...
    if (LOC_IS_DIR(loc) && LOC_HAS_ABSPATH(loc)) {                             \
      ...
      op_ret = posix_pstat(this, (loc)->inode, (loc)->gfid, rpath, iatt_p,     \
                           _gf_false);                                         \
      break;                                                                   \
    }                                                                          \
    errno = 0;                                                                 \
    ...
    if (errno != ELOOP) {                                                      \
      ...
      if (!rpath) {                                                            \
        op_ret = -1;                                                           \
        gf_msg(this->name, GF_LOG_ERROR, errno, P_MSG_INODE_HANDLE_CREATE,     \
...
int posix_handle_path(xlator_t *this, uuid_t gfid, const char *basename,
                      char *buf, size_t len);

int posix_make_ancestryfromgfid(xlator_t *this, char *path, int pathsize,
...
int posix_handle_trash_init(xlator_t *this);
