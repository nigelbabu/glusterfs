...
static inline uint32_t ec_adjust_size_down(ec_t *ec, uint64_t *value,
                                           gf_boolean_t scale) {
  uint64_t head, tmp;
...
static inline int32_t ec_adjust_size_up(ec_t *ec, uint64_t *value,
                                        gf_boolean_t scale) {
  uint64_t tmp;
  int32_t tail;
...
static inline uint32_t ec_adjust_offset_down(ec_t *ec, off_t *value,
                                             gf_boolean_t scale) {
  off_t head, tmp;
 ...
 * different parameter size (off_t vs uint64_t). */
static inline int32_t ec_adjust_offset_up(ec_t *ec, off_t *value,
                                          gf_boolean_t scale) {
  uint64_t tail, tmp;
...
gf_boolean_t ec_is_internal_xattr(dict_t *dict, char *key, data_t *value,
                                  void *data);

void ec_filter_internal_xattrs(dict_t *xattr);

gf_boolean_t ec_is_data_fop(glusterfs_fop_t fop);

int32_t ec_launch_replace_heal(ec_t *ec);
