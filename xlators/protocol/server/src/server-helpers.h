...
call_frame_t *get_frame_from_request(rpcsvc_request_t *req);

int server_connection_cleanup(xlator_t *this, struct _client *client,
                              int32_t flags);
...
inode_t *server_inode_new(inode_table_t *itable, uuid_t gfid);
...
int server_populate_compound_response(xlator_t *this, gfs3_compound_rsp *rsp,
                                      call_frame_t *frame,
                                      compound_args_cbk_t *args_cbk, int index);
int server_get_compound_resolve(server_state_t *state, gfs3_compound_req *req);

int server_populate_compound_request(gfs3_compound_req *req,
                                     call_frame_t *frame,
                                     default_args_t *this_args, int index);
void server_compound_rsp_cleanup(gfs3_compound_rsp *rsp,
                                 compound_args_cbk_t *args);

void server_compound_req_cleanup(gfs3_compound_req *req, int len);

int unserialize_req_locklist(gfs3_setactivelk_req *req,
                             lock_migration_info_t *lmi);

int unserialize_req_locklist_v2(gfx_setactivelk_req *req,
                                lock_migration_info_t *lmi);
...
int server_populate_compound_response_v2(xlator_t *this, gfx_compound_rsp *rsp,
                                         call_frame_t *frame,
                                         compound_args_cbk_t *args_cbk,
                                         int index);
int server_get_compound_resolve_v2(server_state_t *state,
                                   gfx_compound_req *req);

int server_populate_compound_request_v2(gfx_compound_req *req,
                                        call_frame_t *frame,
                                        default_args_t *this_args, int index);
void server_compound_rsp_cleanup_v2(gfx_compound_rsp *rsp,
                                    compound_args_cbk_t *args);

void server_compound_req_cleanup_v2(gfx_compound_req *req, int len);
