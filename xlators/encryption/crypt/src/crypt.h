...
struct rmw_atom {
   ...
   */
  uint32_t (*offset_in)(call_frame_t *frame, struct object_cipher_info *object);
  /*
...
int32_t get_nmtd_link_key(loc_t *loc, struct master_cipher_info *master,
...
extern struct data_cipher_alg data_cipher_algs[LAST_CIPHER_ALG]
                                              [LAST_CIPHER_MODE];
void encrypt_aligned_iov(struct object_cipher_info *object, struct iovec *vec,
                         int count, off_t off);
void decrypt_aligned_iov(struct object_cipher_info *object, struct iovec *vec,
                         int count, off_t off);
int32_t align_iov_by_atoms(xlator_t *this, crypt_local_t *local,
                           struct object_cipher_info *object,
...
int32_t set_config_avec_data(xlator_t *this, crypt_local_t *local,
                             struct avec_config *conf,
                             struct object_cipher_info *object,
                             struct iovec *vec, int32_t vec_count);
int32_t set_config_avec_hole(xlator_t *this, crypt_local_t *local,
                             struct avec_config *conf,
                             struct object_cipher_info *object,
                             glusterfs_fop_t fop);
void set_gap_at_end(call_frame_t *frame, struct object_cipher_info *object,
                    struct avec_config *conf, atom_data_type dtype);
void set_config_offsets(call_frame_t *frame, xlator_t *this, uint64_t offset,
...
int32_t update_format(unsigned char *new, unsigned char *old, size_t old_len,
...
struct rmw_atom *atom_by_types(atom_data_type data,
                               atom_locality_type locality);
void submit_partial(call_frame_t *frame, xlator_t *this, fd_t *fd,
...
static inline int32_t crypt_xlator_id(void) { return CRYPT_XLATOR_ID; }

static inline mtd_loader_id current_mtd_loader(void) { return MTD_LOADER_V1; }
...
static inline uint32_t alg_mode_blkbits(cipher_alg_t alg, cipher_mode_t mode) {
  return data_cipher_algs[alg][mode].blkbits;
}

static inline uint32_t alg_mode_blksize(cipher_alg_t alg, cipher_mode_t mode) {
 ...
 */
static inline int data_local_invariant(crypt_local_t *local) { return 0; }
...
static inline uint32_t nr_calls_full(struct avec_config *conf) {
  ...
  case DATA_ATOM:
    return has_full_blocks(conf) ? logical_blocks_occupied(
                                       0, conf->nr_full_blocks, MAX_IOVEC_BITS)
                                 : 0;
  default:
