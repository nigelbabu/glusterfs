...
#define SVC_ENTRY_POINT_SET(this, xdata, op_ret, op_errno, new_xdata, priv,    \
  ...
  do {                                                                         \
    if (!xdata) {                                                              \
      ...
      if (!new_xdata) {                                                        \
        gf_log(this->name, GF_LOG_ERROR, "failed to allocate new dict");       \
        op_ret = -1;                                                           \
        op_errno = ENOMEM;                                                     \
        goto label;                                                            \
      }                                                                        \
...
#define SVC_GET_SUBVOL_FROM_CTX(this, op_ret, op_errno, inode_type, ret,       \
  ...
  do {                                                                         \
    ...
    if (ret < 0) {                                                             \
      gf_log(this->name, GF_LOG_ERROR, "inode context not found for gfid %s",  \
...
int gf_svc_special_dir_revalidate_lookup(call_frame_t *frame, xlator_t *this,
                                         dict_t *xdata);
