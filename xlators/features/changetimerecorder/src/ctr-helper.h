...
static gf_ctr_local_t *init_ctr_local_t(xlator_t *this) {
...
static void free_ctr_local(gf_ctr_local_t *ctr_local) {
  if (ctr_local)
    mem_put(ctr_local);
}
...
static inline void ctr_free_frame_local(call_frame_t *frame) {
  if (frame) {
 ...
 * */
static inline int set_posix_link_request(xlator_t *this, dict_t **xdata) {
  int ret = -1;
 ...
 *
 * */
static inline gf_boolean_t is_internal_fop(call_frame_t *frame, dict_t *xdata) {
...
#define CTR_IF_FOP_FAILED_THEN_GOTO(this, op_ret, op_errno, label)             \
  do {                                                                         \
    if (op_ret == -1) {                                                        \
      gf_msg_trace(this->name, 0, "Failed fop with %s", strerror(op_errno));   \
      goto label;                                                              \
    };                                                                         \
  } while (0)
...
int fill_db_record_for_unwind(xlator_t *this, gf_ctr_local_t *ctr_local,
                              gfdb_fop_type_t fop_type,
                              gfdb_fop_path_t fop_path);

int fill_db_record_for_wind(xlator_t *this, gf_ctr_local_t *ctr_local,
...
static inline int ctr_insert_wind(call_frame_t *frame, xlator_t *this,
                                  gf_ctr_inode_context_t *ctr_inode_cx) {
  int ret = -1;
  gf_ctr_private_t *_priv = NULL;
  gf_ctr_local_t *ctr_local = NULL;
  ...
  if (_priv->ctr_record_wind && ctr_inode_cx->ia_type != IA_IFDIR) {
    ...
    if (!frame->local) {
      gf_msg(this->name, GF_LOG_ERROR, 0, CTR_MSG_CREATE_CTR_LOCAL_ERROR_WIND,
    ...
    if (!ctr_local->is_internal_fop) {
       ...
       * OR
       * its NOT a metadata fop */
      if ((ctr_inode_cx->is_metadata_fop && _priv->ctr_record_metadata_heat) ||
          (!ctr_inode_cx->is_metadata_fop)) {
    ...
    /* when its a internal FOPS*/
      ...
      CTR_DB_REC(ctr_local).do_record_times =
          (isdentrycreatefop(ctr_inode_cx->fop_type)) ? _gf_true : _gf_false;
    }

    /*Fill the db record for insertion*/
    ret = fill_db_record_for_wind(this, ctr_local, ctr_inode_cx);
    if (ret) {
      gf_msg(this->name, GF_LOG_ERROR, 0, CTR_MSG_FILL_CTR_LOCAL_ERROR_WIND,
 ...
 * call at the end.
 * ****************************************************************************/
static inline int ctr_insert_unwind(call_frame_t *frame, xlator_t *this,
                                    gfdb_fop_type_t fop_type,
                                    gfdb_fop_path_t fop_path) {
  int ret = -1;
  gf_ctr_private_t *_priv = NULL;
  ...
  if (ctr_local && (_priv->ctr_record_unwind || isdentryfop(fop_type)) &&
    ...
    if (ret == -1) {
      gf_msg(this->name, GF_LOG_ERROR, 0, CTR_MSG_FILL_CTR_LOCAL_ERROR_UNWIND,
 ...
 * ****************************************************************************/
static inline int ctr_delete_hard_link_from_db(xlator_t *this, uuid_t gfid,
                                               uuid_t pargfid, char *basename,
                                               gfdb_fop_type_t fop_type,
                                               gfdb_fop_path_t fop_path) {
  int ret = -1;
  gfdb_db_record_t gfdb_db_record;
  ...
  GF_VALIDATE_OR_GOTO(this->name,
                      (fop_path == GFDB_FOP_UNDEL || GFDB_FOP_UNDEL_ALL), out);
...
static inline gf_boolean_t __is_inode_expired(ctr_xlator_ctx_t *ctr_xlator_ctx,
  ...
  ret =
      (time_diff >= _priv->ctr_lookupheal_inode_timeout) ? _gf_true : _gf_false;
  return ret;
}

static inline gf_boolean_t __is_hardlink_expired(ctr_hard_link_t *ctr_hard_link,
                                                 gf_ctr_private_t *_priv,
                                                 gfdb_time_t *current_time) {
  gf_boolean_t ret = _gf_false;
  uint64_t time_diff = 0;
...
add_hard_link_ctx(call_frame_t *frame, xlator_t *this, inode_t *inode) {
  ...
  if (!ctr_xlator_ctx) {
    gf_msg(this->name, GF_LOG_ERROR, 0, CTR_MSG_ACCESS_CTR_INODE_CONTEXT_FAILED,
...
static inline int delete_hard_link_ctx(call_frame_t *frame, xlator_t *this,
                                       inode_t *inode) {
  int ret = -1;
  ctr_xlator_ctx_t *ctr_xlator_ctx = NULL;
  gf_ctr_local_t *ctr_local = NULL;
  ...
  ret =
      ctr_delete_hard_link(this, ctr_xlator_ctx, CTR_DB_REC(ctr_local).pargfid,
                           CTR_DB_REC(ctr_local).file_name);
  if (ret) {
...
static inline int update_hard_link_ctx(call_frame_t *frame, xlator_t *this,
                                       inode_t *inode) {
  int ret = -1;
  ctr_xlator_ctx_t *ctr_xlator_ctx = NULL;
  gf_ctr_local_t *ctr_local = NULL;
