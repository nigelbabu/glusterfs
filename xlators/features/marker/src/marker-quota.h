  ...
  do {                                                                         \
    ...
    if (_gfid != NULL) {                                                       \
      ...
      _ret = snprintf(_tmp_var, QUOTA_KEY_MAX,                                 \
                      QUOTA_XATTR_PREFIX ".%s.%s." CONTRIBUTION, "quota",      \
                      _gfid_unparsed);                                         \
    } else {                                                                   \
      _ret = snprintf(_tmp_var, QUOTA_KEY_MAX,                                 \
...
int32_t mq_req_xattr(xlator_t *, loc_t *, dict_t *, char *, char *);

int32_t mq_xattr_state(xlator_t *, loc_t *, dict_t *, struct iatt);

int mq_initiate_quota_txn(xlator_t *, loc_t *, struct iatt *);

int mq_initiate_quota_blocking_txn(xlator_t *, loc_t *, struct iatt *);

int mq_create_xattrs_txn(xlator_t *this, loc_t *loc, struct iatt *buf);

int32_t mq_reduce_parent_size_txn(xlator_t *, loc_t *, quota_meta_t *,
                                  uint32_t nlink, call_stub_t *stub);

int32_t mq_forget(xlator_t *, quota_inode_ctx_t *);
#endif
