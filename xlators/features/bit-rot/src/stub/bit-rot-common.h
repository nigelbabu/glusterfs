...
static inline br_vxattr_status_t br_version_xattr_state(dict_t *xattr,
                                                        br_version_t **obuf,
                                                        br_signature_t **sbuf,
                                                        gf_boolean_t *objbad) {
  int32_t ret = 0;
...
static inline int br_is_signature_type_valid(int8_t signaturetype) {
  return ((signaturetype > BR_SIGNATURE_TYPE_ZERO) &&
          (signaturetype < BR_SIGNATURE_TYPE_MAX));
}

static inline void br_set_default_ongoingversion(br_version_t *buf,
                                                 uint32_t *tv) {
...
static inline void br_set_default_signature(br_signature_t *buf, size_t *size) {
...
static inline void br_set_ongoingversion(br_version_t *buf,
...
static inline void br_set_signature(br_signature_t *buf, br_isignature_t *sign,
