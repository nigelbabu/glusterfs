...
#define GLUSTERD_GET_VOLUME_DIR(path, volinfo, priv)                           \
  do {                                                                         \
    ...
    if (volinfo->is_snap_volume) {                                             \
      _vol_dir_len = snprintf(path, PATH_MAX, "%s/snaps/%s/%s", priv->workdir, \
                              volinfo->snapshot->snapname, volinfo->volname);  \
    } else {                                                                   \
      _vol_dir_len = snprintf(path, PATH_MAX, "%s/vols/%s", priv->workdir,     \
                              volinfo->volname);                               \
    }                                                                          \
...
#define GLUSTERD_GET_TIER_PID_FILE(path, volinfo, priv)                        \
  do {                                                                         \
    ...
    _tier_pid_len = snprintf(path, PATH_MAX, "%s/run/%s-tierd.pid", tier_path, \
                             volinfo->volname);                                \
...
#define GLUSTERD_GET_VOLUME_PID_DIR(path, volinfo, priv)                       \
  do {                                                                         \
    ...
    if (volinfo->is_snap_volume) {                                             \
      _vol_pid_len = snprintf(path, PATH_MAX, "%s/snaps/%s/%s", priv->rundir,  \
                              volinfo->snapshot->snapname, volinfo->volname);  \
    } else {                                                                   \
      _vol_pid_len = snprintf(path, PATH_MAX, "%s/vols/%s", priv->rundir,      \
                              volinfo->volname);                               \
    }                                                                          \
...
#define GLUSTERD_GET_SNAP_GEO_REP_DIR(path, snap, priv)                        \
  do {                                                                         \
    ...
    _snap_geo_len = snprintf(path, PATH_MAX, "%s/snaps/%s/%s", priv->workdir,  \
...
#define GLUSTERD_GET_BRICK_DIR(path, volinfo, priv)                            \
  do {                                                                         \
    ...
    if (volinfo->is_snap_volume) {                                             \
      ...
      _brick_len = snprintf(path, PATH_MAX, "%s/%s/%s/%s", priv->workdir,      \
...
#define GLUSTERD_GET_TMP_PATH(abspath, path)                                   \
  do {                                                                         \
    snprintf(abspath, sizeof(abspath) - 1, DEFAULT_VAR_RUN_DIRECTORY "/tmp%s", \
             path);                                                            \
  } while (0)
...
#define GLUSTERD_GET_NFS_PIDFILE(pidfile, nfspath, priv)                       \
  do {                                                                         \
    ...
    _nfs_pid_len =                                                             \
        snprintf(pidfile, PATH_MAX, "%s/nfs/nfs.pid", priv->rundir);           \
...
#define GLUSTERD_GET_QUOTAD_PIDFILE(pidfile, quotadpath, priv)                 \
  do {                                                                         \
    ...
    _quotad_pid_len =                                                          \
        snprintf(pidfile, PATH_MAX, "%s/quotad/quotad.pid", priv->rundir);     \
...
#define GLUSTERD_GET_QUOTA_CRAWL_PIDDIR(piddir, volinfo, type)                 \
  do {                                                                         \
    ...
    if (type == GF_QUOTA_OPTION_TYPE_ENABLE ||                                 \
      ...
      _crawl_pid_len =                                                         \
          snprintf(piddir, PATH_MAX, "%s/run/quota/enable", _volpath);         \
    else                                                                       \
      _crawl_pid_len =                                                         \
          snprintf(piddir, PATH_MAX, "%s/run/quota/disable", _volpath);        \
...
#define GLUSTERD_GET_DEFRAG_DIR(path, volinfo, priv)                           \
  do {                                                                         \
    ...
    _defrag_dir_len = snprintf(path, PATH_MAX, "%s/%s", vol_path, operation);  \
...
#define GLUSTERD_GET_DEFRAG_SOCK_FILE_OLD(path, volinfo, priv)                 \
  do {                                                                         \
    ...
    _sockfile_old_len = snprintf(path, PATH_MAX, "%s/%s.sock", defrag_path,    \
                                 uuid_utoa(MY_UUID));                          \
...
#define GLUSTERD_GET_DEFRAG_SOCK_FILE(path, volinfo)                           \
  do {                                                                         \
    ...
    _defrag_sockfile_len = snprintf(                                           \
        path, UNIX_PATH_MAX, DEFAULT_VAR_RUN_DIRECTORY "/gluster-%s-%s.sock",  \
        operation, uuid_utoa(volinfo->volume_id));                             \
...
#define GLUSTERD_GET_DEFRAG_PID_FILE(path, volinfo, priv)                      \
  do {                                                                         \
    ...
    GLUSTERD_GET_DEFRAG_DIR(defrag_path, volinfo, priv);                       \
    _defrag_pidfile_len = snprintf(path, PATH_MAX, "%s/%s.pid", defrag_path,   \
                                   uuid_utoa(MY_UUID));                        \
...
static inline unsigned char *__glusterd_uuid() {
...
int glusterd_big_locked_cbk(struct rpc_req *req, struct iovec *iov, int count,
                            void *myframe, fop_cbk_fn_t fn);

int glusterd_big_locked_handler(rpcsvc_request_t *req, rpcsvc_actor actor_fn);

int32_t glusterd_brick_from_brickinfo(glusterd_brickinfo_t *brickinfo,
                                      char **new_brick);
int glusterd_probe_begin(rpcsvc_request_t *req, const char *hoststr, int port,
                         dict_t *dict, int *op_errno);

int glusterd_xfer_friend_add_resp(rpcsvc_request_t *req, char *myhostname,
                                  char *remote_hostname, int port,
                                  int32_t op_ret, int32_t op_errno);

int glusterd_friend_add(const char *hoststr, int port,
                        glusterd_friend_sm_state_t state, uuid_t *uuid,
                        glusterd_peerinfo_t **friend, gf_boolean_t restore,
                        glusterd_peerctx_args_t *args);

int glusterd_friend_add_from_peerinfo(glusterd_peerinfo_t *friend,
                                      gf_boolean_t restore,
                                      glusterd_peerctx_args_t *args);
int glusterd_friend_rpc_create(xlator_t *this, glusterd_peerinfo_t *peerinfo,
                               glusterd_peerctx_args_t *args);
int glusterd_friend_remove(uuid_t uuid, char *hostname);

int glusterd_op_lock_send_resp(rpcsvc_request_t *req, int32_t status);

int glusterd_op_unlock_send_resp(rpcsvc_request_t *req, int32_t status);

int glusterd_op_mgmt_v3_lock_send_resp(rpcsvc_request_t *req, uuid_t *txn_id,
                                       int32_t status);

int glusterd_op_mgmt_v3_unlock_send_resp(rpcsvc_request_t *req, uuid_t *txn_id,
                                         int32_t status);

int glusterd_op_stage_send_resp(rpcsvc_request_t *req, int32_t op,
                                int32_t status, char *op_errstr,
                                dict_t *rsp_dict);

int glusterd_op_commmit_send_resp(rpcsvc_request_t *req, int32_t op,
                                  int32_t status);

int32_t glusterd_create_volume(rpcsvc_request_t *req, dict_t *dict);

int glusterd_handle_incoming_friend_req(rpcsvc_request_t *req);

int glusterd_handle_probe_query(rpcsvc_request_t *req);

int glusterd_handle_cluster_lock(rpcsvc_request_t *req);

int glusterd_handle_cluster_unlock(rpcsvc_request_t *req);

int glusterd_handle_stage_op(rpcsvc_request_t *req);

int glusterd_handle_commit_op(rpcsvc_request_t *req);

int glusterd_handle_cli_probe(rpcsvc_request_t *req);

int glusterd_handle_create_volume(rpcsvc_request_t *req);

int glusterd_handle_defrag_volume(rpcsvc_request_t *req);

int glusterd_handle_defrag_volume_v2(rpcsvc_request_t *req);

int glusterd_xfer_cli_probe_resp(rpcsvc_request_t *req, int32_t op_ret,
                                 int32_t op_errno, char *op_errstr,
                                 char *hostname, int port, dict_t *dict);

int glusterd_op_commit_send_resp(rpcsvc_request_t *req, int32_t op,
                                 int32_t status, char *op_errstr,
                                 dict_t *rsp_dict);

int glusterd_xfer_friend_remove_resp(rpcsvc_request_t *req, char *hostname,
                                     int port);

int glusterd_deprobe_begin(rpcsvc_request_t *req, const char *hoststr, int port,
                           uuid_t uuid, dict_t *dict, int *op_errno);

int glusterd_handle_cli_deprobe(rpcsvc_request_t *req);

int glusterd_handle_incoming_unfriend_req(rpcsvc_request_t *req);

int32_t glusterd_list_friends(rpcsvc_request_t *req, dict_t *dict,
                              int32_t flags);

int glusterd_handle_cli_list_friends(rpcsvc_request_t *req);

int glusterd_handle_cli_start_volume(rpcsvc_request_t *req);

int glusterd_handle_friend_update(rpcsvc_request_t *req);

int glusterd_handle_cli_stop_volume(rpcsvc_request_t *req);

int glusterd_handle_cli_delete_volume(rpcsvc_request_t *req);

int glusterd_handle_cli_get_volume(rpcsvc_request_t *req);

int32_t glusterd_get_volumes(rpcsvc_request_t *req, dict_t *dict,
                             int32_t flags);

int glusterd_handle_add_brick(rpcsvc_request_t *req);

int glusterd_handle_tier(rpcsvc_request_t *req);

int glusterd_handle_attach_tier(rpcsvc_request_t *req);

int glusterd_handle_detach_tier(rpcsvc_request_t *req);

int glusterd_handle_add_tier_brick(rpcsvc_request_t *req);

int glusterd_handle_replace_brick(rpcsvc_request_t *req);

int glusterd_handle_remove_brick(rpcsvc_request_t *req);

int glusterd_handle_log_rotate(rpcsvc_request_t *req);

int glusterd_handle_sync_volume(rpcsvc_request_t *req);

int glusterd_defrag_start_validate(glusterd_volinfo_t *volinfo, char *op_errstr,
                                   size_t len, glusterd_op_t op);

int glusterd_rebalance_cmd_validate(int cmd, char *volname,
                                    glusterd_volinfo_t **volinfo,
                                    char *op_errstr, size_t len);

int32_t glusterd_log_filename(rpcsvc_request_t *req, dict_t *dict);

int32_t glusterd_log_rotate(rpcsvc_request_t *req, dict_t *dict);

int32_t glusterd_remove_brick(rpcsvc_request_t *req, dict_t *dict);

int32_t glusterd_set_volume(rpcsvc_request_t *req, dict_t *dict);

int32_t glusterd_reset_volume(rpcsvc_request_t *req, dict_t *dict);

int32_t glusterd_gsync_set(rpcsvc_request_t *req, dict_t *dict);

int32_t glusterd_quota(rpcsvc_request_t *req, dict_t *dict);

int glusterd_handle_set_volume(rpcsvc_request_t *req);

int glusterd_handle_reset_volume(rpcsvc_request_t *req);

int glusterd_handle_copy_file(rpcsvc_request_t *req);

int glusterd_handle_sys_exec(rpcsvc_request_t *req);

int glusterd_handle_gsync_set(rpcsvc_request_t *req);

int glusterd_handle_quota(rpcsvc_request_t *req);

int glusterd_handle_bitrot(rpcsvc_request_t *req);

int glusterd_handle_fsm_log(rpcsvc_request_t *req);

int glusterd_handle_reset_brick(rpcsvc_request_t *req);

int glusterd_xfer_cli_deprobe_resp(rpcsvc_request_t *req, int32_t op_ret,
                                   int32_t op_errno, char *op_errstr,
                                   char *hostname, dict_t *dict);

int glusterd_client_statedump_submit_req(char *volname, char *target_ip,
                                         char *pid);

int glusterd_fetchspec_notify(xlator_t *this);

int glusterd_fetchsnap_notify(xlator_t *this);

int glusterd_add_tier_volume_detail_to_dict(glusterd_volinfo_t *volinfo,
                                            dict_t *volumes, int count);

int glusterd_add_volume_detail_to_dict(glusterd_volinfo_t *volinfo,
                                       dict_t *volumes, int count);

int glusterd_restart_bricks();

int32_t glusterd_volume_txn(rpcsvc_request_t *req, char *volname, int flags,
                            glusterd_op_t op);

int glusterd_peer_dump_version(xlator_t *this, struct rpc_clnt *rpc,
                               glusterd_peerctx_t *peerctx);

int glusterd_validate_reconfopts(glusterd_volinfo_t *volinfo, dict_t *val_dict,
                                 char **op_errstr);
int glusterd_handle_cli_profile_volume(rpcsvc_request_t *req);

int glusterd_handle_getwd(rpcsvc_request_t *req);

int32_t glusterd_set_volume(rpcsvc_request_t *req, dict_t *dict);
int glusterd_peer_rpc_notify(struct rpc_clnt *rpc, void *mydata,
                             rpc_clnt_event_t event, void *data);
int glusterd_brick_rpc_notify(struct rpc_clnt *rpc, void *mydata,
                              rpc_clnt_event_t event, void *data);

int glusterd_rpc_create(struct rpc_clnt **rpc, dict_t *options,
...
int glusterd_handle_defrag_start(glusterd_volinfo_t *volinfo, char *op_errstr,
                                 ...
                                 glusterd_op_t op);
int glusterd_rebalance_rpc_create(glusterd_volinfo_t *volinfo);
...
int glusterd_handle_snapshot(rpcsvc_request_t *req);
...
int glusterd_op_gsync_args_get(dict_t *dict, char **op_errstr, char **master,
...
int32_t glusterd_op_begin_synctask(rpcsvc_request_t *req, glusterd_op_t op,
                                   void *dict);
int32_t glusterd_defrag_event_notify_handle(dict_t *dict);
...
/* snapshot */
glusterd_snap_t *glusterd_new_snap_object();

int32_t glusterd_list_add_snapvol(glusterd_volinfo_t *origin_vol,
                                  glusterd_volinfo_t *snap_vol);

glusterd_snap_t *glusterd_remove_snap_by_id(uuid_t snap_id);

glusterd_snap_t *glusterd_remove_snap_by_name(char *snap_name);

glusterd_snap_t *glusterd_find_snap_by_name(char *snap_name);

glusterd_snap_t *glusterd_find_snap_by_id(uuid_t snap_id);

int glusterd_snapshot_prevalidate(dict_t *dict, char **op_errstr,
                                  dict_t *rsp_dict, uint32_t *op_errno);
int glusterd_snapshot_brickop(dict_t *dict, char **op_errstr, dict_t *rsp_dict);
int glusterd_snapshot(dict_t *dict, char **op_errstr, uint32_t *op_errno,
                      dict_t *rsp_dict);
int glusterd_snapshot_postvalidate(dict_t *dict, int32_t op_ret,
                                   char **op_errstr, dict_t *rsp_dict);
char *glusterd_build_snap_device_path(char *device, char *snapname,
                                      int32_t brick_count);

int32_t glusterd_snap_remove(dict_t *rsp_dict, glusterd_snap_t *snap,
                             gf_boolean_t remove_lvm, gf_boolean_t force,
                             gf_boolean_t is_clone);
int32_t glusterd_snapshot_cleanup(dict_t *dict, char **op_errstr,
                                  dict_t *rsp_dict);

int32_t glusterd_add_missed_snaps_to_list(dict_t *dict,
                                          int32_t missed_snap_count);

int32_t glusterd_add_new_entry_to_list(char *missed_info, char *snap_vol_id,
                                       int32_t brick_num, char *brick_path,
                                       int32_t snap_op, int32_t snap_status);

int glusterd_snapshot_revert_restore_from_snap(glusterd_snap_t *snap);

int glusterd_add_brick_status_to_dict(dict_t *dict, glusterd_volinfo_t *volinfo,
                                      glusterd_brickinfo_t *brickinfo,
                                      char *key_prefix);

int32_t glusterd_handle_snap_limit(dict_t *dict, dict_t *rsp_dict);

gf_boolean_t glusterd_should_i_stop_bitd();

int glusterd_remove_brick_migrate_cbk(glusterd_volinfo_t *volinfo,
                                      gf_defrag_status_t status);
/* tier */

int __glusterd_handle_reset_brick(rpcsvc_request_t *req);
...
int glusterd_op_remove_tier_brick(dict_t *dict, char **op_errstr,
                                  dict_t *rsp_dict);
int glusterd_tier_prevalidate(dict_t *dict, char **op_errstr, dict_t *rsp_dict,
                              uint32_t *op_errno);

int glusterd_options_init(xlator_t *this);
#endif
