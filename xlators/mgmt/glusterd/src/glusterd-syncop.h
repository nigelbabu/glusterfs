...
#define GD_SYNCOP(rpc, stb, cookie, cbk, req, prog, procnum, xdrproc)          \
  do {                                                                         \
    ...
    ret = gd_syncop_submit_request(rpc, req, stb, cookie, prog, procnum, cbk,  \
                                   (xdrproc_t)xdrproc);                        \
    if (!ret)                                                                  \
...
int32_t gd_syncop_brick_op_cbk(struct rpc_req *req, struct iovec *iov,
...
int gd_syncop_mgmt_stage_op(glusterd_peerinfo_t *peerinfo, struct syncargs *arg,
...
void gd_synctask_barrier_wait(struct syncargs *args, int count);

int gd_brick_op_phase(glusterd_op_t op, dict_t *op_ctx, dict_t *req_dict,
                      char **op_errstr);

int glusterd_syncop_aggr_rsp_dict(glusterd_op_t op, dict_t *aggr, dict_t *rsp);

void gd_syncargs_init(struct syncargs *args, dict_t *op_ctx);
