...
int glusterd_op_commit_hook(glusterd_op_t op, dict_t *op_ctx,
                            glusterd_commit_hook_type_t type);

int glusterd_op_sm_new_event(glusterd_op_sm_event_type_t event_type,
                             glusterd_op_sm_event_t **new_event);
int glusterd_op_sm_inject_event(glusterd_op_sm_event_type_t event_type,
                                uuid_t *txn_id, void *ctx);

int glusterd_op_sm_init();

int glusterd_op_sm();

int32_t glusterd_op_set_ctx(void *ctx);

int32_t glusterd_op_set_op(glusterd_op_t op);

int glusterd_op_build_payload(dict_t **req, char **op_errstr, dict_t *op_ctx);

int32_t glusterd_op_stage_validate(glusterd_op_t op, dict_t *req,
                                   char **op_errstr, dict_t *rsp_dict);

int32_t glusterd_op_commit_perform(glusterd_op_t op, dict_t *req,
                                   char **op_errstr, dict_t *dict);

int32_t glusterd_op_txn_begin(rpcsvc_request_t *req, glusterd_op_t op,
                              void *ctx, char *err_str, size_t err_len);

int32_t glusterd_op_txn_complete();

void *glusterd_op_get_ctx();

int32_t glusterd_op_set_req(rpcsvc_request_t *req);

int32_t glusterd_op_send_cli_response(glusterd_op_t op, int32_t op_ret,
                                      int32_t op_errno, rpcsvc_request_t *req,
                                      void *ctx, char *op_errstr);
int32_t glusterd_op_get_op();

int32_t glusterd_op_clear_op();

int32_t glusterd_op_free_ctx(glusterd_op_t op, void *ctx);

int glusterd_check_option_exists(char *optstring, char **completion);

int set_xlator_option(dict_t *dict, char *key, char *value);

char *glusterd_op_sm_state_name_get(int state);

char *glusterd_op_sm_event_name_get(int event);
int32_t glusterd_op_bricks_select(glusterd_op_t op, dict_t *dict,
                                  char **op_errstr,
                                  struct cds_list_head *selected,
                                  dict_t *rsp_dict);
int glusterd_brick_op_build_payload(glusterd_op_t op,
                                    glusterd_brickinfo_t *brickinfo,
                                    gd1_mgmt_brick_op_req **req, dict_t *dict);
int glusterd_node_op_build_payload(glusterd_op_t op,
                                   gd1_mgmt_brick_op_req **req, dict_t *dict);
int32_t glusterd_handle_brick_rsp(void *pending_entry, glusterd_op_t op,
                                  dict_t *rsp_dict, dict_t *ctx_dict,
                                  char **op_errstr, gd_node_type type);

dict_t *glusterd_op_init_commit_rsp_dict(glusterd_op_t op);

void glusterd_op_modify_op_ctx(glusterd_op_t op, void *op_ctx);

void glusterd_op_perform_detach_tier(glusterd_volinfo_t *volinfo);

int glusterd_set_detach_bricks(dict_t *dict, glusterd_volinfo_t *volinfo);

int32_t glusterd_volume_stats_read_perf(char *brick_path, int32_t blk_size,
                                        int32_t blk_count, double *throughput,
                                        double *time);
int32_t glusterd_volume_stats_write_perf(char *brick_path, int32_t blk_size,
                                         int32_t blk_count, double *throughput,
                                         double *time);
gf_boolean_t glusterd_is_volume_started(glusterd_volinfo_t *volinfo);

int glusterd_start_bricks(glusterd_volinfo_t *volinfo);

gf_boolean_t glusterd_are_all_volumes_stopped();
int glusterd_stop_bricks(glusterd_volinfo_t *volinfo);
int glusterd_defrag_volume_node_rsp(dict_t *req_dict, dict_t *rsp_dict,
                                    dict_t *op_ctx);
#ifdef HAVE_BD_XLATOR
int glusterd_is_valid_vg(glusterd_brickinfo_t *brick, int check_tag, char *msg);
#endif

int32_t glusterd_get_txn_opinfo(uuid_t *txn_id, glusterd_op_info_t *opinfo);

int32_t glusterd_set_txn_opinfo(uuid_t *txn_id, glusterd_op_info_t *opinfo);

int32_t glusterd_clear_txn_opinfo(uuid_t *txn_id);

int32_t glusterd_generate_txn_id(dict_t *dict, uuid_t **txn_id);

void glusterd_set_opinfo(char *errstr, int32_t op_errno, int32_t op_ret);

int glusterd_dict_set_volid(dict_t *dict, char *volname, char **op_errstr);

int32_t glusterd_tier_op(xlator_t *this, void *data);
#endif
