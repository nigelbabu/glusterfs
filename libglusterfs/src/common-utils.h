...
static inline void BIT_SET(unsigned char *array, unsigned int index) {
...
static inline void BIT_CLEAR(unsigned char *array, unsigned int index) {
  unsigned int offset = index / 8;
  unsigned int shift = index % 8;
...
static inline unsigned int BIT_VALUE(unsigned char *array, unsigned int index) {
  unsigned int offset = index / 8;
  unsigned int shift = index % 8;
...
#define GF_REMOVE_INTERNAL_XATTR(pattern, dict)                                \
  do {                                                                         \
    if (!dict) {                                                               \
      gf_msg(this->name, GF_LOG_ERROR, 0, LG_MSG_DICT_NULL, "dict is null");   \
      break;                                                                   \
    }                                                                          \
...
#define GF_IF_INTERNAL_XATTR_GOTO(pattern, dict, op_errno, label)              \
  do {                                                                         \
    ...
    if (dict_foreach_fnmatch(dict, pattern, dict_null_foreach_fn, NULL) > 0) { \
      op_errno = EPERM;                                                        \
...
static inline void iov_free(struct iovec *vector, int count) {
  int i;
...
static inline int iov_length(const struct iovec *vector, int count) {
  int i = 0;
  size_t size = 0;
...
static inline struct iovec *iov_dup(const struct iovec *vector, int count) {
  int bytecount = 0;
  int i;
...
static inline int iov_subset(struct iovec *orig, int orig_count,
...
static inline void iov_unload(char *buf, const struct iovec *vector,
                              int count) {
  int i;
  int copied = 0;
...
static inline size_t iov_load(const struct iovec *vector, int count, char *buf,
                              int size) {
  size_t left = size;
  size_t cp = 0;
  int ret = 0;
...
static inline size_t iov_copy(const struct iovec *dst, int dcnt,
...
static inline int mem_0filled(const char *buf, size_t size) {
  int i = 0;
  int ret = 0;
...
static inline int iov_0filled(struct iovec *vector, int count) {
  int i = 0;
  int ret = 0;
...
static inline char *gf_time_fmt(char *dst, size_t sz_dst, time_t utime,
                                unsigned int fmt) {
...
int mkdir_p(char *path, mode_t mode, gf_boolean_t allow_symlinks);
/*
...
gf_boolean_t gf_ports_reserved(char *blocked_port, unsigned char *ports,
                               uint32_t ceiling);
...
int gf_thread_create(pthread_t *thread, const pthread_attr_t *attr,
                     ...
                     const char *name);
int gf_thread_create_detached(pthread_t *thread, void *(*start_routine)(void *),
                              void *arg, const char *name);
gf_boolean_t gf_is_pid_running(int pid);
gf_boolean_t gf_is_service_running(char *pidfile, int *pid);
gf_boolean_t gf_valid_pid(const char *pid, int length);
int gf_skip_header_section(int fd, int header_len);

struct iatt;
struct _dict;

gf_boolean_t dht_is_linkfile(struct iatt *buf, struct _dict *dict);

int gf_check_log_format(const char *value);

int gf_check_logger(const char *value);

gf_boolean_t gf_compare_sockaddr(const struct sockaddr *addr1,
...
int32_t gf_build_absolute_path(char *current_path, char *relative_path,
                               char **path);
...
int glusterfs_is_local_pathinfo(char *pathinfo, gf_boolean_t *local);
...
gf_boolean_t gf_is_zero_filled_stat(struct iatt *buf);

void gf_zero_fill_stat(struct iatt *buf);

gf_boolean_t gf_is_valid_xattr_namespace(char *k);
...
int glusterfs_compute_sha256(const unsigned char *content, size_t size,
                             char *sha256_hash);
