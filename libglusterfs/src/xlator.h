...
xlator_t *file_to_xlator_tree(glusterfs_ctx_t *ctx, FILE *fp);
...
void xlator_foreach(xlator_t *this, void (*fn)(xlator_t *each, void *data),
                    void *data);
...
void xlator_set_inode_lru_limit(xlator_t *this, void *data);

void inode_destroy_notify(inode_t *inode, const char *xlname);

int loc_copy(loc_t *dst, loc_t *src);
int loc_copy_overload_parent(loc_t *dst, loc_t *src, inode_t *parent);
...
gf_boolean_t is_graph_topology_equal(glusterfs_graph_t *graph1,
                                     glusterfs_graph_t *graph2);
int glusterfs_volfile_reconfigure(FILE *newvolfile_fp, glusterfs_ctx_t *ctx);

int gf_volfile_reconfigure(int oldvollen, FILE *newvolfile_fp,
                           glusterfs_ctx_t *ctx, const char *oldvolfile);

int loc_touchup(loc_t *loc, const char *name);

int glusterfs_leaf_position(xlator_t *tgt);

int glusterfs_reachable_leaves(xlator_t *base, dict_t *leaves);

int xlator_subvolume_count(xlator_t *this);

void xlator_init_lock(void);
void xlator_init_unlock(void);
int copy_opts_to_child(xlator_t *src, xlator_t *dst, char *glob);

int glusterfs_delete_volfile_checksum(glusterfs_ctx_t *ctx,
                                      const char *volfile_id);
int xlator_memrec_free(xlator_t *xl);

void xlator_mem_cleanup(xlator_t *this);
#endif /* _XLATOR_H */
