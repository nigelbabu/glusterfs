...
int32_t gf_store_mkdir(char *path);

int32_t gf_store_handle_create_on_absence(gf_store_handle_t **shandle,
                                          char *path);

int32_t gf_store_mkstemp(gf_store_handle_t *shandle);

int gf_store_sync_direntry(char *path);

int32_t gf_store_rename_tmppath(gf_store_handle_t *shandle);

int32_t gf_store_unlink_tmppath(gf_store_handle_t *shandle);

int gf_store_read_and_tokenize(FILE *file, char *str, int size, char **iter_key,
                               char **iter_val,
                               gf_store_op_errno_t *store_errno);

int32_t gf_store_retrieve_value(gf_store_handle_t *handle, char *key,
                                char **value);
...
int gf_store_handle_retrieve(char *path, gf_store_handle_t **handle);

int32_t gf_store_handle_destroy(gf_store_handle_t *handle);

int32_t gf_store_iter_new(gf_store_handle_t *shandle, gf_store_iter_t **iter);

int32_t gf_store_validate_key_value(char *storepath, char *key, char *val,
                                    gf_store_op_errno_t *op_errno);

int32_t gf_store_iter_get_next(gf_store_iter_t *iter, char **key, char **value,
                               gf_store_op_errno_t *op_errno);

int32_t gf_store_iter_get_matching(gf_store_iter_t *iter, char *key,
                                   char **value);

int32_t gf_store_iter_destroy(gf_store_iter_t *iter);

char *gf_store_strerror(gf_store_op_errno_t op_errno);
