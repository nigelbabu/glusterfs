...
static inline uint32_t ia_major(uint64_t ia_dev) {
  return (uint32_t)(ia_dev >> 32);
}

static inline uint32_t ia_minor(uint64_t ia_dev) {
  return (uint32_t)(ia_dev & 0xffffffff);
}

static inline uint64_t ia_makedev(uint32_t ia_maj, uint32_t ia_min) {
  return ((((uint64_t)ia_maj) << 32) | ia_min);
}

static inline ia_prot_t ia_prot_from_st_mode(mode_t mode) {
  ia_prot_t ia_prot = {
      0,
  };
...
static inline ia_type_t ia_type_from_st_mode(mode_t mode) {
...
static inline uint32_t st_mode_prot_from_ia(ia_prot_t prot) {
  uint32_t prot_bit = 0;
...
static inline mode_t st_mode_from_ia(ia_prot_t prot, ia_type_t type) {
  mode_t st_mode = 0;
  uint32_t type_bit = 0;
  uint32_t prot_bit = 0;
...
static inline int iatt_from_stat(struct iatt *iatt, struct stat *stat) {
  ...
  */
  ...
  iatt->ia_flags = iatt->ia_flags | IATT_TYPE | IATT_MODE | IATT_NLINK |
                   IATT_UID | IATT_GID | IATT_SIZE | IATT_BLOCKS | IATT_ATIME |
                   IATT_MTIME | IATT_CTIME;

  return 0;
}

static inline int iatt_to_stat(struct iatt *iatt, struct stat *stat) {
...
static inline void oldiatt_from_iatt(struct old_iatt *o_iatt,
                                     struct iatt *c_iatt) {
...
static inline void iatt_from_oldiatt(struct iatt *c_iatt,
                                     struct old_iatt *o_iatt) {
  ...
  c_iatt->ia_flags = IATT_TYPE | IATT_MODE | IATT_NLINK | IATT_INO | IATT_UID |
                     IATT_GID | IATT_SIZE | IATT_BLOCKS | IATT_ATIME |
                     IATT_MTIME | IATT_CTIME | IATT_GFID;

  return;
}

static inline int is_same_mode(ia_prot_t prot1, ia_prot_t prot2) {
  int ret = 0;
