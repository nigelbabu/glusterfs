 ...
 * */
static inline gf_loglevel_t _gfdb_log_level(gf_loglevel_t given_level,
...
static inline uint64_t gfdb_time_2_usec(gfdb_time_t *gfdb_time) {
  GF_ASSERT(gfdb_time);
...
static inline int gf_string2gfdbdbsync(char *sync_option) {
  int ret = -1;
...
static inline int gf_string2gfdbdbtype(char *db_option) {
  int ret = -1;
...
static inline gf_boolean_t iswindpath(gfdb_fop_path_t gfdb_fop_path) {
  return ((gfdb_fop_path == GFDB_FOP_WIND) || (gfdb_fop_path == GFDB_FOP_WDEL))
             ? _gf_true
             : _gf_false;
}

static inline gf_boolean_t isunwindpath(gfdb_fop_path_t gfdb_fop_path) {
...
static inline gf_boolean_t isreadfop(gfdb_fop_type_t fop_type) {
  return (fop_type & GFDB_FOP_READ_OP) ? _gf_true : _gf_false;
}

static inline gf_boolean_t isdentryfop(gfdb_fop_type_t fop_type) {
...
static inline gf_boolean_t isdentrycreatefop(gfdb_fop_type_t fop_type) {
