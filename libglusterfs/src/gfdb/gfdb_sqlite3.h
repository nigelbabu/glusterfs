...
#define CHECK_SQL_CONN(sql_conn, out)                                          \
  do {                                                                         \
    ...
    if (!sql_conn->sqlite3_db_conn) {                                          \
      gf_msg(GFDB_STR_SQLITE3, GF_LOG_ERROR, 0, LG_MSG_CONNECTION_INIT_FAILED, \
...
#define GF_SQLITE3_SET_PRAGMA(sqlite3_config_str, param_key, format, value,    \
  ...
  do {                                                                         \
    ...
    ret = sqlite3_exec(sql_conn->sqlite3_db_conn, sqlite3_config_str, NULL,    \
                       NULL, NULL);                                            \
    if (ret != SQLITE_OK) {                                                    \
...
static inline int gfdb_set_sql_params(char *comp_name, dict_t *from_dict,
                                      dict_t *to_dict) {
...
int gf_sqlite3_find_unchanged_for_time_freq(
    void *db_conn, gf_query_callback_t query_callback, void *_query_cbk_args,
    gfdb_time_t *for_time, int write_freq_cnt, int read_freq_cnt,
    gf_boolean_t clear_counters);
int gf_sqlite3_find_recently_changed_files_freq(
    void *db_conn, gf_query_callback_t query_callback, void *_query_cbk_args,
 ...
 *      On failure return -1
 * */
int gf_sqlite3_set_pragma(void *db_conn, char *pragma_key, char *pragma_value);
 ...
 *      On failure return -1
 * */
int gf_sqlite3_vacuum(void *db_conn, gf_boolean_t compact_active,
