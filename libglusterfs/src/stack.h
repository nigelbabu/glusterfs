...
static inline void FRAME_DESTROY(call_frame_t *frame) {
  void *local = NULL;
...
static inline void STACK_DESTROY(call_stack_t *stack) {
...
static inline void STACK_RESET(call_stack_t *stack) {
  call_frame_t *frame = NULL;
  call_frame_t *tmp = NULL;
...
static inline void *get_the_pt_fop(void *base_fop, int fop_idx) {
...
static inline int call_stack_alloc_groups(call_stack_t *stack, int ngrps) {
  if (ngrps <= SMALL_GROUP_COUNT) {
    stack->groups = stack->groups_small;
  } else {
    stack->groups_large =
        GF_CALLOC(ngrps, sizeof(gid_t), gf_common_mt_groups_t);
...
static inline int call_frames_count(call_stack_t *call_stack) {
  call_frame_t *pos;
  int32_t count = 0;
...
static inline call_frame_t *copy_frame(call_frame_t *frame) {
