...
static inline size_t posix_acl_xattr_size(unsigned int count) {
  return (sizeof(posix_acl_xattr_header) +
          (count * sizeof(posix_acl_xattr_entry)));
}

static inline ssize_t posix_acl_xattr_count(size_t size) {
...
static inline const char *gf_posix_acl_get_key(const acl_type_t type) {
  char *acl_key = NULL;
...
static inline const acl_type_t gf_posix_acl_get_type(const char *key) {
  acl_type_t type = 0;
