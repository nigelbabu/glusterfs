...
int parser_set_string(struct parser *parser, const char *complete_str);

/* Frees memory used by the string after all matches are found */
int parser_unset_string(struct parser *parser);

/* Free memory used by the parser */
void parser_deinit(struct parser *ptr);

/* Get the next matching string */
char *parser_get_next_match(struct parser *parser);
