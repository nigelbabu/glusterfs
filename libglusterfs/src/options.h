...
int xlator_option_info_list(volume_opt_list_t *list, char *key, char **def_val,
                            char **descr);
/*
...
volume_option_t *xlator_volume_option_get(xlator_t *xl, const char *key);

volume_option_t *xlator_volume_option_get_list(volume_opt_list_t *vol_list,
                                               const char *key);
...
#define DECLARE_RECONF_OPT(type_t, type)                                       \
  int xlator_option_reconf_##type(xlator_t *this, dict_t *options, char *key,  \
                                  type_t *val_p);
...
#define DEFINE_RECONF_OPT(type_t, type, conv)                                  \
  int xlator_option_reconf_##type(xlator_t *this, dict_t *options, char *key,  \
    ...
    if (value == def_value) {                                                  \
      gf_msg_trace(this->name, 0, "option %s using default value %s", key,     \
                   value);                                                     \
    } else {                                                                   \
      gf_msg(this->name, GF_LOG_INFO, 0, 0, "option %s using set value %s",    \
             key, value);                                                      \
    }                                                                          \
    old_THIS = THIS;                                                           \
    THIS = this;                                                               \
